...
def msg_type_to_msg_format(msg_type):
    format1_list = ['join', 'disconnect']
    format2_list = ['request_users_list', 'err_unknown_message', 'err_server_full', 'err_username_unavailable']
    format3_list = ['response_users_list']
    format4_list = ['send_message', 'forward_message', 'forward_file', 'send_file']
    
    if msg_type in format1_list:
        return 1
    if msg_type in format2_list:
        return 2
    if msg_type in format3_list:
        return 3
    if msg_type in format4_list:
        return 4


def validate_user_input(user_input):
    user_input_list = user_input.split(' ')
    allowed_user_input = ['list', 'quit', 'help', 'msg', 'file']
    if user_input_list[0] in allowed_user_input:
        return True
    return False


def input_to_std_msg_type(user_input):
    user_input_list = user_input.split(' ')
    if user_input_list[0] == "list":
        return "request_users_list"
    if user_input_list[0] == "quit":
        return "disconnect"
    if user_input_list[0] == "msg":
        return "send_message"
    if user_input_list[0] == "file":
        return "send_file"